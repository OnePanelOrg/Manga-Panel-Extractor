...
class Debug:
	...
	def html(self, images_dir, reldir):
		...
		for i in range(len(self.steps)-1):
			...
			self.infos['panels'] = list(map(lambda p: p.to_xywh_path(), self.steps[i]['panels']))
			json1 = self.infos.copy()
			self.infos['panels'] = list(map(lambda p: p.to_xywh_path(), self.steps[j]['panels']))
